...
pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/Arati654/custompolicy.git'
            }
        }
            ...
            steps {
                withSonarQubeEnv('sonarserver') {         // 'sonarserver' must be configured under SonarQube servers
                    sh '''
                        ...
                        -Dsonar.sources=. \
                        -Dsonar.host.url=http://20.83.164.184:9000 \
        ...
        stage('Login to Azure') {
            steps {
                withCredentials([
                    ...
                    string(credentialsId: 'azure-tentant', variable: 'AZURE_TENANT')
                ]) {
                    sh '''
